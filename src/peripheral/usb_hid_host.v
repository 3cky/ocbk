...
module usb_hid_host (
    ...
    // LOCAL HOOKS (ocbk): two observability outputs for hardware bring-up.
    // dbg_regs is upstream's own `// assign dbg_regs = regs;` line made real
    // (an unpacked array cannot be assigned whole in Verilog, hence the
    // concatenation below); dev_connected exposes ukp's connect flag so a
    // dead port is distinguishable from a device that attached but failed to
    // enumerate. Neither carries any logic change.
    output [55:0] dbg_regs,		// {protocol,subclass,class,PID_H,PID_L,VID_H,VID_L}
    output        dev_connected
);
...
// LOCAL HOOK (ocbk): upstream's commented `assign dbg_regs = regs;`, made real.
assign dbg_regs     = {regs[6], regs[5], regs[4], regs[3], regs[2], regs[1], regs[0]};
assign dev_connected = connected;
