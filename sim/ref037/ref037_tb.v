...
// +romprog: the same program words placed in ROM at 101000 (fetches from mask
// ROM, fixed N_ROM reply, no 037 stealing; data accesses still hit RAM).
...
//
// The program is one word table; default mode copies it to RAM at 001000
// (FAIL park 001124, self-loop 001136), +romprog copies the SAME words to ROM
// at 101000 (park 101124, loop 101136). The words are position-independent:
// immediates keep pointing at RAM 002000, so in ROM mode the fetches come from
// the fixed-timing ROM while the data traffic still exercises the stolen RAM.
...
initial begin
   ...
   // value self-check - a wrong RMW result parks in the FAIL loop, which
   // changes the fetch trace and breaks the golden diff.
   ...
   if ($test$plusargs("romprog")) begin
      rom[0] = 16'o000137; rom[1] = 16'o101000;          // JMP @#101000
