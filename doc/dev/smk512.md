...
- **SMK512 (Phase 8; DIP 8): 512 KB segmented RAM (increment 1) + BIOS ROM
  ...
  **WARM-RESET RECOVERY PREAMBLE + WHOLE-LADDER RETRY (kept from the
  ...
  A **third** joined them in Phase 15 (2026-08-16): `bk_total_q` +
  `media_ok_q`. `lba_a`'s load mux is this design's chronic critical
  ENDPOINT — it was already the worst setup path on `main` (from
  `g_val`, +0.118 ns), and a +23 LE increment elsewhere in the design
  (the USB gamepad) re-placed the fitter enough to promote the
  `sd_backend|bk_total` leg of the same mux to **+0.021 ns**. Registering
  the 28-bit capacity locally moved that whole family to **+0.787** and
  took the endpoint out of the critical set entirely — worst setup
  +0.255, i.e. *better than the baseline it started from*. **The pairing
  is the correctness argument, and the trap is the mirror of
  `enable_q`'s**: `sd_backend` raises `bk_total` and `bk_media_ok` on the
  SAME cycle, so gating `G_WAIT` on the raw flag while reading the
  registered capacity would read a stale zero on the one cycle that
  matters and attach a zero-sector drive. `bk_media_ok` is used in
  exactly one place and never drops mid-run, so both are delayed
  together and both reset to 0 — here the safe direction.
